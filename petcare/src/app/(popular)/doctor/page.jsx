...
const Page = () => {
  ...
  const submitData = async () => {
    try {
      const response = await axios.post('http://127.0.0.1:8000/doctorlogin/', payload);
      setLoginData(response.data);
      if (response.data.message === 'Login successfull') {
        setStatus(1);
        setUser(response.data.data);
        sessionStorage.setItem('user', JSON.stringify(response.data.data));
        router.push('/doctor/booking');
      } else {
        setStatus(2);
        setShowAlert(true);
      }
  ...
  return (<div className="bg-gray-100 flex items-center justify-center h-[800px]">
    ...
    <div className="bg-white p-8 rounded-xl shadow-lg max-w-sm w-full text-center">
      <h2 className="text-2xl font-bold mb-4 text-gray-800">Login For Doctor</h2>
