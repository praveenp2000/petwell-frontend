...
const Page = () => {
    ...
    const storeAdoption = async () => {
      try {
        const response = await axios.post(
          'http://127.0.0.1:8000/getallpetservice/',
          payload
        );
  ...
  if (petserviceData.length === 0) return <LoadingScreen />;
  ...
  return (
    <>
      <div className='flex justify-between my-auto font-[Poppins] w-full'>
        <h4 className='text-center text-[#ECDFCC]'>Pet Service Details</h4>
      ...
      <div className='table-container w-full font-[Poppins]'>
        <table>
          <thead>
            <tr>
              <th>No</th>
              <th className='w-[250px]'>Pet Id</th>
              <th className='w-[250px]'>Service Id</th>
              <th>Status</th>
              <th>Action</th>
            </tr>
          </thead>
          <tbody>
            {petserviceData.data.map((data, index) => (
              <tr key={index}>
                <td>{index + 1}</td>
                <td>{data.pet_id}</td>
        ...
        {editOpen && (
          <EditModal
            title='Edit Pet Service Details'
            open={true}
