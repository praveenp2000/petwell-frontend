...
const Page = () => {
    ...
    const storeAdoption = async () => {
      try {
        const response = await axios.post(
          'http://127.0.0.1:8000/getallservice/',
          payload
        );
  ...
  if (serviceData.length === 0) return <LoadingScreen />;
  ...
  return (
    <>
      <div className='flex justify-between my-auto font-[Poppins] w-full'>
        <h4 className='text-center text-[#ECDFCC]'>Service List</h4>
        ...
        <table>
          <thead>
            <tr>
              ...
              <th className='w-[250px]'>Cost</th>
              <th className='w-[250px]'>Time</th>
              <th>Action</th>
            </tr>
          </thead>
          <tbody>
            {serviceData.data.map((data, index) => (
              <tr key={index}>
                <td>{index + 1}</td>
                <td>{data.name}</td>
                <td>{data.cost}</td>
