...
const Page = () => {
    ...
    const storeAdoption = async () => {
      try {
        const response = await axios.post(
          'http://127.0.0.1:8000/getallhealth/',
          payload
        );
  ...
  if (healthData.length === 0) return <LoadingScreen />;
  ...
  return (
    <>
      <div className='flex justify-between my-auto font-[Poppins] w-full'>
        <h4 className='text-center text-[#ECDFCC]'>Health Details</h4>
      ...
      <div className='table-container w-full font-[Poppins]'>
        <table>
          ...
          <tbody>
            {healthData.data.map((data, index) => (
              <tr key={index}>
                <td>{index + 1}</td>
